		...
		foreach($arResult['ITEMS'] as $key => $arRecipe){
			if($key < 24){
				$Html .= '<div class="item recipe_list_item">
							<div class="photo"><a href="/detail/'.($arRecipe['CODE'] ? $arRecipe['CODE'] : $arRecipe['ID']).'/" class="'.$arRecipe['NAME'].'"><img src="'.$arRecipe['PREVIEW_PICTURE']['SRC'].'" width="'.$arRecipe['PREVIEW_PICTURE']['WIDTH'].'" height="'.$arRecipe['PREVIEW_PICTURE']['HEIGHT'].'" alt="'.$arRecipe['PREVIEW_PICTURE']['DESCRIPTION'].'"></a></div>
							<h5><a href="/detail/'.($arRecipe['CODE'] ? $arRecipe['CODE'] : $arRecipe['ID']).'/">'.$arRecipe['NAME'].'</a></h5>
							<p class="author">От: '.$arRecipe['USER']['FULLNAME'].'</p>
							<p class="info">
								<span class="comments_icon"><noindex><a href="/detail/'.($arRecipe['CODE'] ? $arRecipe['CODE'] : $arRecipe['ID']).'/#comments">'.intval($arRecipe['PROPERTY_COMMENT_COUNT_VALUE']).'</a></noindex></span>
							</p>
						</div>';
			}else{
