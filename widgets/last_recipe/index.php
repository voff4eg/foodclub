...
$rsLastRecipe = CIBlockElement::GetList(array("created"=>"desc"),array("IBLOCK_ID"=>5,"!PROPERTY_lib"=>false),false,array("nTopCount"=>6),array("ID","NAME","PREVIEW_PICTURE"));
while($arLastRecipe = $rsLastRecipe->GetNext()){
	$arFile = CFile::GetFileArray($arLastRecipe["PREVIEW_PICTURE"]);
	if($arFile) {
		echo '<div class="recipe"><a href="/detail/'.$arLastRecipe["ID"].'/" target="_blank" class="recipe_photo"><img src="'.$arFile["SRC"].'" width="'.$arFile["WIDTH"].'" height="'.$arFile["HEIGHT"].'" alt="'.$arLastRecipe["NAME"].'" /></a><br /><a href="/detail/'.$arLastRecipe["ID"].'" target="_blank" class="recipe_name">'.$arLastRecipe["NAME"].'</a></div>';
	}
}
?>
</div>
