...
if(CModule::IncludeModule("iblock")){
	...
	//Рецепты
	$rsRecipes = CIBlockElement::GetList(array(),array("IBLOCK_ID"=>5,"PROPERTY_lib"=>"Y","PROPERTY_dish_type" => 38,array(
        "LOGIC" => "OR",
	...
	//echo "@".count($arRecipeSteps)."@";die;
	$arIngredientIDs = array();
	...
	while($obRecipeStage = $rsRecipeStages->GetNextElement()){
		...
		if(!empty($arRecipeStage["PROPERTIES"]["ingredient"]["VALUE"])){
			$arIngredientIDs = array_merge($arIngredientIDs, $arRecipeStage["PROPERTIES"]["ingredient"]["VALUE"]);	
		}
		echo "@#".count($arIngredientIDs)."#@";
	...
	$arIngredientIDs = array_unique($arIngredientIDs);
	...
	if(!empty($arIngredientIDs)){
		$content .= 'CREATE TABLE ingredients (ccal NUMERIC, measure TEXT, name TEXT, _id INTEGER PRIMARY KEY);'.PHP_EOL;
		//Ингредиенты
		$rsIngredients = CIBlockElement::GetList(array(),array("IBLOCK_ID"=>3,"ID" => $arIngredientIDs),false,false,array("ID","NAME","PREVIEW_PICTURE","PROPERTY_unit","PROPERTY_kkal"));
	...
	$fp = fopen($_SERVER['DOCUMENT_ROOT']."/upload/sql4saladsapp.txt", 'w+');
	fwrite($fp, $content);
	fclose($fp);
}