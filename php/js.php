...
foreach($arRecipes as $recipe){
	$arJSONRecipe["id"] = ($recipe['CODE'] ? $recipe['CODE'] : $recipe['ID']);
...
foreach($arDishes as $Dish)
{
	...
	$arDish["image"] = CFile::GetPath($Dish['PREVIEW_PICTURE']);//CFile::GetPath($Dish['PREVIEW_PICTURE']['ID']);
	$arDishesJSON[] = $arDish;
}
...
if(!isset($arIngGroups)){
	$arIngGroups = $CFClub->getUnitList("s1","js");
}
...
// /search_service/?id=$& /ingredient/$&/
//echo "</pre>";
$arRecipesHeads = array("title" => "Рецепты", "url" => "/detail/$&/", "items" => $arRecipesJSON);
$arIngredientsHeads = array("title" => "Ингредиенты", "url" => "/$&", "items" => $arIngredientsJSON);
$arIngGroupsHeads = array("title" => "Группы ингредиентов", "url" => "", "items" => $arIngGroupsJSON);
$arCuisinesHeads = array("title" => "Кухни", "url" => "/recipes/?cuisine=$&", "items" => $arCuisineJSON);
$arDishesHeads = array("title" => "Типы блюд", "url" => "/recipes/?dish=$&", "items" => $arDishesJSON);
