...
if(intval($_REQUEST["vote"]) > 0){
	if(intval($arParams["CONTEST"]) > 0){
		$contest = $arParams["CONTEST"];
	}
	CModule::IncludeModule("iblock");
	if ($USER->IsAuthorized()){
		$rsVote = CIBlockElement::GetList(array(),array("IBLOCK_ID"=>16,"NAME"=>$USER->GetID()."й пользователь","PROPERTY_contest"=>$contest),false,false,array("ID","NAME","PREVIEW_TEXT","PROPERTY_recipe"));
		while($Vote = $rsVote->GetNext()){
			$Person = array("ID" => $Vote["ID"], "NAME" => $Vote["NAME"]);
			$arVotes[] = $Vote["PROPERTY_RECIPE_VALUE"];
		}
		if(!empty($arVotes)){
			if(!in_array(intval($_REQUEST["vote"]),$arVotes)){
				$arVotes[] = intval($_REQUEST["vote"]);
				...
				$res = $el->Update($Person["ID"], $arLoadProductArray);
			}
		}else{
			$arVotes = array();
			$el = new CIBlockElement;
			...
			$PROP = array();
			$PROP[34] = $contest;
			$PROP[35] = intval($_REQUEST["vote"]);
			
			$arLoadProductArray = Array(
			  "MODIFIED_BY"    => $USER->GetID(), // элемент изменен текущим пользователем
			  "IBLOCK_SECTION_ID" => false,          // элемент лежит в корне раздела
			  "IBLOCK_ID"      => 16,
			  "NAME"           => $user_id."й пользователь",
			  "PROPERTY_VALUES"=> $PROP,
			  "ACTIVE"         => "Y",            // активен
			  );
...
if($this->StartResultCache(false, array($user_id,$voted,($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		$rsVote = CIBlockElement::GetList(array(),array("IBLOCK_ID"=>16,"NAME"=>$USER->GetID()."й пользователь","PROPERTY_contest"=>$contest),false,false,array("ID","NAME","PREVIEW_TEXT","PROPERTY_recipe"));
		while($Vote = $rsVote->GetNext()){
			$arResult["VOTED"][] = $Vote["PROPERTY_RECIPE_VALUE"];
		}
		if(intval($arParams["CONTEST"]) > 0){
			$arFilter["PROPERTY_contest"] = $arParams["CONTEST"];
		}
		$arSelect[] = "PROPERTY_comment_count";
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			$arItem["LOGIN"] = CUser::GetById($arItem["CREATED_BY"])->Fetch();
			$rsCountVotes = CIBlockElement::GetList(array(), array("IBLOCK_ID"=>16,"PROPERTY_recipe"=>$arItem["ID"]), array("PROPERTY_recipe"), false, array("ID","NAME"));
			while($arCountVotes = $rsCountVotes->GetNext()){
				$arItem["VOTES_COUNT"] = $arCountVotes["CNT"];
			}
