...
<div class="comments_block">
	...
	<?} else {?>
		...
		<div class="comment foodclub">
			...
			<div class="padding">
				<div class="text">
					Если Вы хотите оставить комментарий, Вам необходимо <noindex><a href="/auth/?backurl=/blogs/group/<?=$arResult['Blog']['SOCNET_GROUP_ID']?>/blog/<?=$arResult['Post']['ID']?>/">авторизоваться</a></noindex> или <noindex><a href="/registration/?backurl=/blogs/group/<?=$arResult['Blog']['SOCNET_GROUP_ID']?>/blog/<?=$arResult['Post']['ID']?>/">зарегистрироваться</a></noindex> на сайте.
				</div>
			</div>
		</div>
...
function getHTML($arComment, $arResult){
	...
	if(strlen($arComment['arUser']["NAME"]) > 0 && strlen($arComment['arUser']["LAST_NAME"]) > 0){
		$arComment['arUser']['FULLNAME'] = $arComment['arUser']["NAME"]." ".$arComment['arUser']["LAST_NAME"];
	}else{
		$arComment['arUser']["FULLNAME"] = $arComment['arUser']['LOGIN'];	 	
	}
	ob_start();?>
		<div class="comment<?if($USER->isAuthorized() && $USER->GetId() == $arComment['arUser']['ID'] ){?> mine<?}?>" id="<?=$arComment['ID']?>">
			<div class="icons">
				...
				<div class="photo">
					<div class="big_photo">
						<div>
							<img src="<?=$arAvatar['SRC']?>" width="100" height="100" alt="<?=$arComment['arUser']['FULLNAME']?>">
						</div>
					</div>
					<img src="<?=$arAvatar['SRC']?>" width="30" height="30" alt="<?=$arComment['arUser']['FULLNAME']?>">
				</div>
