...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		$arFilter['PROPERTY_brand.ID']=$_REQUEST['value'];
		//$arFilter['PROPERTY_tech_type.ID']=$_REQUEST['type'];
		
		//$arFilter['!PROPERTY_users']=$USER->GetID();
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			//print_r($arItem['PROPERTIES']['users']['VALUE']);
			if(!in_array($USER->GetID(), $arItem['PROPERTIES']['users']['VALUE']))
