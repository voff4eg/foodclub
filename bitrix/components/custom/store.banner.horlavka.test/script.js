...
function filterListEvents() {
	...
	function clickListAnchor(e) {
		...
		var $span = $filter.find(".b-filter__item__button.open .bg span");
		//$filter.find(".b-filter__item__button.open .bg span").text($this.text());
		cutLine({
			text: $this.text(),
			bgWidth: $span.parent().width(),
			$container: $span
		});
...
function cutLine(data) {
	...
	console.log(data.bgWidth);
	while(data.text.length > 3 && data.$container.outerWidth() > data.bgWidth) {
