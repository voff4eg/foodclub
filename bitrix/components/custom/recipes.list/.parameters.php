...
if(!class_exists("CFClub")){
	require_once($_SERVER["DOCUMENT_ROOT"]."/classes/main.class.php");
}
$CFClub = new CFClub();
$arKitchens = $CFClub->getKitchens();
if(!empty($arKitchens)){
	foreach($arKitchens as $kitchen){
		$KitchenList[ $kitchen["ID"] ] = $kitchen["NAME"];
	}
}
$arMainIngredients = $CFClub->getMainIgredientList();
if(!empty($arMainIngredients)){
	foreach($arMainIngredients as $mainingredient){
		$MainIngredientList[ $mainingredient["ID"] ] = $mainingredient["NAME"];
	}
}
$arDishTypes = $CFClub->getDishType();
if(!empty($arDishTypes)){
	foreach($arDishTypes as $dishtype){
		$DishTypeList[ $dishtype["ID"] ] = $dishtype["NAME"];
	}
}
$arTags = $CFClub->getTags();
if(!empty($arTags)){
...
$arComponentParameters = array(
	...
	"PARAMETERS" => array(
		...
		"MAIN_INGREDIENT_LIST" => array(
			"PARENT" => "BASE",
			"NAME" => GetMessage("MAIN_INGREDIENTS"),
			"TYPE" => "LIST",
			"MULTIPLE" => "Y",
			"VALUES" => $MainIngredientList,
		),
		"DISHTYPE_LIST" => array(
			"PARENT" => "BASE",
			"NAME" => GetMessage("DISH_TYPES"),
			"TYPE" => "LIST",
			"MULTIPLE" => "Y",
			"VALUES" => $DishTypeList,
		),
