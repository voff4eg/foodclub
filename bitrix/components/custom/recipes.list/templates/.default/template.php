...
<?foreach($arResult["ITEMS"] as $i => $arItem):?>
	...
	<div class="item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">	
		<?if($arParams["DISPLAY_PICTURE"]!="N" && is_array($arItem["PREVIEW_PICTURE"])):?>
			<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
				<a href="<?=($arItem["CODE"] ? "/detail/".$arItem["CODE"]."/" : $arItem["DETAIL_PAGE_URL"])?>"><img src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>" width="<?=$arItem["PREVIEW_PICTURE"]["WIDTH"]?>" height="<?=$arItem["PREVIEW_PICTURE"]["HEIGHT"]?>" alt="<?=$arItem["NAME"]?>" title="<?=$arItem["NAME"]?>" /></a>				
			<?else:?>
		...
		<?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
			<?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>				
				<h3><a href="<?=($arItem["CODE"] ? "/detail/".$arItem["CODE"]."/" : $arItem["DETAIL_PAGE_URL"])?>"><?echo $arItem["NAME"]?></a></h3>
			<?else:?>				
	...
	<?if(($i % 4) == 3):?><div class="i-clearfix"></div><?endif;?>
<?endforeach;?>
</div>
