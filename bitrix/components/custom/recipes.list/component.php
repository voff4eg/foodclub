...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		if(intval($arParams["KITCHEN_LIST"])){
			$arFilter["PROPERTY_kitchen"] = $arParams["KITCHEN_LIST"];
		}
		if(intval($arParams["MAIN_INGREDIENT_LIST"])){
			$arFilter["PROPERTY_main_ingredient"] = $arParams["MAIN_INGREDIENT_LIST"];
		}
		if(intval($arParams["DISHTYPE_LIST"])){
			$arFilter["PROPERTY_dish_type"] = $arParams["DISHTYPE_LIST"];
		}
		if(intval($arParams["TAG_LIST"])){
			$arFilter["?TAGS"] = $arParams["TAG_LIST"];
		}
		$arFilter["PROPERTY_lib"] = "y";

		if(!empty($arParams["INGREDIENT_ID"])){
			$arRecipeID = array();
			$rsIngredient = CIBlockElement::GetList(array(),array("IBLOCK_ID" => "3", "ID" => $arParams["INGREDIENT_ID"]));
			if($obIngredient = $rsIngredient->GetNextElement()){
				$arIngredient = $obIngredient->GetFields();
				$arIngredient["PROPERTIES"] = $obIngredient->GetProperties();
			}
			if(!empty($arIngredient["PROPERTIES"]["similar"]["VALUE"])){
				$arIngredientID = array_merge($arIngredient["PROPERTIES"]["similar"]["VALUE"],array($arParams["INGREDIENT_ID"]));
			}else{
				$arIngredientID = $arParams["INGREDIENT_ID"];
			}
			$rsStages = CIBlockElement::GetList(array(),array("IBLOCK_ID" => 4, "PROPERTY_ingredient" => $arIngredientID, "!PROPERTY_parent" => false),array("PROPERTY_parent"),false);
			while($arStage = $rsStages -> Fetch()){
				$arRecipeID[] = $arStage["PROPERTY_PARENT_VALUE"];
			}
			$arRecipeID = array_unique($arRecipeID);
			if(!empty($arRecipeID)){
				$arFilter["ID"] = $arRecipeID;
			}else{
				$this->AbortResultCache();
				return;
			}
