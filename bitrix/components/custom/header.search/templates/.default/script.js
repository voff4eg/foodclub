...
$(function() {
	/*Top_smartsearch*/
	$("#recipe_search_field").keyup(function(event) {
		if (window.event) event = window.event;
		switch (event.keyCode ? event.keyCode : event.which ? event.which : null) {
			case 38:
				if(keyPress == 0) {topSsNavUp(this);
				}
				break;
			case 40:
				if(keyPress == 0) {topSsNavDown(this);
				}
				break;
			case 13:
				if(keyPress == 0) {
			...
			break;
			default:topSsFunction(this);
		}
	});
	$("#recipe_search_field").keypress(function(event) {
		if (window.event) event = window.event;
		switch (event.keyCode ? event.keyCode : event.which ? event.which : null) {
			case 38:
				keyPress = 1;
	...
	if(document.getElementById("top_search_list")) {
		$("#top_search_list ul.top_search_list").css({
			top:pageY(document.getElementById("recipe_search_field")) - pageY($("#top_search_list")[0])+41,
			left:pageX(document.getElementById("recipe_search_field")) - pageX($("#top_search_list")[0])
		});
	}
	/*_Top_smartsearch_*/
...
function topSsFunction(inputObject) {
	...
	if($(inputObject).attr("value") != "") {
		...
		$("#top_search_list").children("ul").append('<li class="search_all"><a href="/search/' + $(inputObject).attr("value") + '/">Все рецепты с &laquo;<span>' + $(inputObject).attr("value") + '</span>&raquo;</a></li>');
		$("#top_search_list").children("ul").append('<li class="search_all"><a href="/posts_search/?q=' + $(inputObject).attr("value") + '">Все записи с &laquo;<span>' + $(inputObject).attr("value") + '</span>&raquo;</a></li>');
...
function addSsListItem(array, searchString, searchFlag, dataType) {
	...
	for (var i = 0; i < itemNum; i++) {
		...
		$("#top_search_list").children("ul").append('<li><a href="' + href + '">' + array[i] + '</a></li>');
	}
	return searchFlag;
}
