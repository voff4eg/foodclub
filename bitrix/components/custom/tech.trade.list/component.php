...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $USER->GetId())))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		$arFilter['PROPERTY_tech_type.ID']=$_REQUEST['value'];
		//$arFilter['PROPERTY_tech_type.ID']=$USER->GetId();
		$arFilter['!PROPERTY_users']=$USER->GetId();//echo $USER->GetId();
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			foreach($arParams["PROPERTY_CODE"] as $pid)
			{
				...
				/////////ПОЛУЧЕНИЕ СВОЙСТВ ПРОИЗВОДИТЕЛЯ/////////
				$arItem1['id']= $arItem["PROPERTIES"]['brand']['VALUE'];
			
				$res = CIBlockElement::GetByID($arItem1['id']);
				if($ar_res = $res->GetNext())
				$arItem1['name']= $ar_res['NAME'];
			...
			$arResult["ITEMS"]['trade'][$arItem1['id']] = $arItem1;
			$arResult["ITEMS"]['models'][$arItem1['id']][] = $el;
