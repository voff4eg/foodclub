...
if(!empty($arBlog) && $arBlog["ACTIVE"] == "Y" && $arGroup["SITE_ID"] == SITE_ID)
{
	...
	if(!empty($arPost) && $arBlog["ID"] == $arPost["BLOG_ID"])
	{
		...
		{
			if(CUser::IsAdmin()){
				echo "@".intval(check_bitrix_sessid() && (!$bSoNet && (CBlogPost::CanUserDeletePost(IntVal($arParams["ID"]), ($USER->IsAuthorized() ? $arResult["USER_ID"] : 0 )) || CUser::IsAdmin()) || ($bSoNet && (CBlogSoNetPost::CanUserDeletePost(IntVal($arParams["ID"]), $user_id, $arParams["USER_ID"], $arParams["SOCNET_GROUP_ID"]) || CUser::IsAdmin()))))."@";
			}
			if (/*check_bitrix_sessid() && */(!$bSoNet && (CBlogPost::CanUserDeletePost(IntVal($arParams["ID"]), ($USER->IsAuthorized() ? $arResult["USER_ID"] : 0 )) || CUser::IsAdmin()) || ($bSoNet && (CBlogSoNetPost::CanUserDeletePost(IntVal($arParams["ID"]), $user_id, $arParams["USER_ID"], $arParams["SOCNET_GROUP_ID"]) || CUser::IsAdmin()))))
			{
				if ($bSoNet)
			...
			if(!empty($arPost) && $arPost["PUBLISH_STATUS"] != BLOG_PUBLISH_STATUS_DRAFT)
			{
				if($arPost["PUBLISH_STATUS"] == "P" || $arResult["PostPerm"] == BLOG_PERMS_FULL || $arPost["AUTHOR_ID"] == $arResult["USER_ID"])
				{
					...
					if ($arParams["CACHE_TIME"] > 0 && $cache->InitCache($arParams["CACHE_TIME"], $cache_id, $cache_path))
					{
						...
						$APPLICATION->AddHeadString('<meta property="og:title" content="'.$arResult["Post"]["TITLE"].'"/>',true);
						$APPLICATION->AddHeadString('<meta property="og:type" content="food"/>',true);
						$APPLICATION->AddHeadString('<meta property="og:image" content="http://'.$_SERVER["SERVER_NAME"].$arResult["image"].'" />',true);
						...
						if(strlen(strip_tags($arResult["Post"]["DETAIL_TEXT"])) > 0)
						{
							...
							if($arPost["DETAIL_TEXT_TYPE"] == "html" && COption::GetOptionString("blog","allow_html", "N") == "Y")
							{
								...
								$arResult["Post"]["metaog"] = $p->convert($arPost["~DETAIL_TEXT"], false, $arImages, $arAllow, $arParserParams);
							}
							else
							{
							...
							$APPLICATION->AddHeadString('<meta property="og:description" content="'.mb_substr(strip_tags($arResult["Post"]["metaog"]), 0, 200).'"/>',true);
						}
						$cache->Output();
					}
					else
					{
						...
						//	Получаем ID первого фото поста
						foreach($arImages as $imageID => $fileID){
							if(strstr(strstr($arResult["Post"]["DETAIL_TEXT"], "ID=".$imageID , true), "ID=") === false){
								$arResult["image"] = CFile::GetPath($fileID);
								break;
							}
						}
						...
						if($arResult["PostPerm"] > BLOG_PERMS_MODERATE || ($arResult["PostPerm"]>=BLOG_PERMS_WRITE && $arPost["AUTHOR_ID"] == $arResult["USER_ID"]) || CUser::IsAdmin())
