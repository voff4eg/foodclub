...
<h2>Зарегистрироваться</h2>
	<div class="left_column">
		...
		<?if(strlen($arResult["BACKURL"]) > 0){?><input type="hidden" name="backurl" value="<?=$arResult["BACKURL"]?>" /><?}?>
			...
			<div class="form_field">
				<label>Подтверждение пароля <span class="no_text">?</span></label>
				<input type="password" class="text" name="REGISTER[CONFIRM_PASSWORD]" value="<?=$arResult["VALUES"]["CONFIRM_PASSWORD"]?>">
			</div>
			<div class="form_field<?=(strpos($errors,"Пользователь с таким e-mail") ? " attention" : "" )?>">
				<label><?=(strpos($errors,"Пользователь с таким e-mail") ? "Пользователь с таким e-mail ".$arResult["VALUES"]["EMAIL"]." уже существует" : "E-mail" )?> <span class="no_text">?</span></label>
			...
			?>
			<div class="submit"><button class="button" type="submit">Зарегистрироваться</button></div>
		</form>
	</div>
