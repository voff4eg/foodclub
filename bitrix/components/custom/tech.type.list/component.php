...
if($this->StartResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		///Сверва делаем выборку элементов и смотрим, каким типа техники(31) он принадлежат. 
		///Таким образом определяем список типов техники, в которых есть элемкнты. Затем его и получаем.
		$massTypes=array(); 
		$arSelectModel[]="PROPERTY_tech_type";
		$arSelectModel[]="ID";
		$arSelectModel[]="IBLOCK_ID";
		$arFilterModel = array (
			"IBLOCK_ID" => 33,
			"IBLOCK_LID" => SITE_ID,
			"ACTIVE" => "Y",
		);
		$rsElementModel = CIBlockElement::GetList($arSort, $arFilterModel, false, $arNavParams, $arSelectModel);
		while($obElementModel = $rsElementModel->GetNextElement())
		{
			//$arItemModel = $obElementModel->GetFields();
			$arItemModel = $obElementModel->GetProperties();
			$massTypes[$arItemModel['tech_type']['VALUE']]=$arItemModel['tech_type']['VALUE'];
			//print_r($arItemModel);
			//echo '<br><br>';
		
		}
		//die;
		foreach($massTypes as $tElem)
		{
			$arFilterElems[]=$tElem;
		}
	//	print_r($massTypes);
