...
$arHeader = Array();
$arHeader[] = array("id"=>"NAME", "content"=>GetMessage("IBLOCK_ELEMENT_ADMIN_NAME"), "sort"=>"name", "default"=>true);

$arHeader[] = array("id"=>"ACTIVE", "content"=>GetMessage("IBLOCK_ELEMENT_ADMIN_ACTIVE"), "sort"=>"active", "default"=>true, "align"=>"center");
...
$arHeader[] = array("id"=>"SORT", "content"=>GetMessage("IBLOCK_ELEMENT_ADMIN_SORT"), "sort"=>"sort", "default"=>true, "align"=>"right");
$arHeader[] = array("id"=>"TIMESTAMP_X", "content"=>GetMessage("IBLOCK_ELEMENT_ADMIN_TIMESTAMP"), "sort"=>"timestamp_x", "default"=>true);
$arHeader[] = array("id"=>"USER_NAME", "content"=>GetMessage("IBLOCK_MODIFIED_BY"), "sort"=>"modified_by");
...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	{
		$rsProperties = CIBlockElement::GetProperty($IBLOCK_ID, $f_ID);
			...
			else //published
			{
				...
				if(strlen($f_DETAIL_PAGE_URL) > 0)
				{
					$arActions[] = array(
						...
						"TEXT"=>GetMessage("IBLOCK_EL_ADMIN_VIEW"),
						"TITLE"=>GetMessage("IBEL_A_ORIG"),
						"ACTION"=>$lAdmin->ActionRedirect(htmlspecialchars(CIBlock::ReplaceDetailUrl($arRes_orig["DETAIL_PAGE_URL"], $arRes_orig)))
					);
				}
...
if($arIBTYPE["SECTIONS"]=="Y")
{
	...
	if(intval($find_section_section)>0)
	{
		...
		while($nav->ExtractFields("nav_"))
			...
			{
				$sLastFolder = 'iblock_section_admin.php?type='.htmlspecialchars($type).'&amp;lang='.LANG.'&amp;IBLOCK_ID='.$IBLOCK_ID.'&amp;find_section_section='.$nav_ID;
...
?>
	...
	<tr>
		<td><?echo GetMessage("IBLOCK_F_NAME")?></td>
