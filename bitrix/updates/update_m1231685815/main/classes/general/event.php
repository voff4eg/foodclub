...
class CAllEvent
{
	...
	function HandleEvent($arEvent)
	{
		...
		{
			$rsMail = $DB->Query("
				SELECT ID, SUBJECT, MESSAGE, EMAIL_FROM, EMAIL_TO, BODY_TYPE, BCC
				FROM b_event_message M
			...
			while ($event = $events->Fetch())
				ExecuteModuleEvent($event, &$arFields, &$db_mail_result_array);

			$email_from = CAllEvent::ReplaceTemplate($db_mail_result_array["EMAIL_FROM"], $arFields);
			...
			{
				$email_from = CAllEvent::EncodeMimeString($email_from, $charset);
