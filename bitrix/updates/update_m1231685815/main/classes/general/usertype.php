	...
	*/
				...
				$js = "
					...
					<!--
						function addNewRow(tableID, regexp, rindex)
						{
							...
							var oCell = oRow.insertCell(0);
							var html = tbl.rows[cnt-2].cells[0].innerHTML;
							new_html =  html.replace(regexp,
								function(html)
								{
									return html.replace('['+arguments[rindex]+']', '['+(1+parseInt(arguments[rindex]))+']');
								}
							);
							oCell.innerHTML = new_html;
						}
					//-->
					</script>
					";
	...
	function GetRights($ENTITY_ID=false, $ID=false)
	{
		...
		else
		{
			...
			while($arEvent = $db_events->Fetch())
			{
				$res = ExecuteModuleEvent($arEvent, $ENTITY_ID);
				if($res > $RIGHTS)
					$RIGHTS = $res;
			}
		}
					...
					switch($FIELD_TYPE)
					{
						...
						case "datetime":
							...
							switch($operation)
							{
								...
								case "B":
									if(is_array($FIELD_VALUE) && count($FIELD_VALUE)>1)
										$result[] = $FIELD_NAME." between ".$FIELD_VALUE." AND ".$FIELD_VALUE;
								...
								case "NB":
									if(is_array($FIELD_VALUE) && count($FIELD_VALUE)>1)
										$result[] = $FIELD_NAME." not between ".$FIELD_VALUE." AND ".$FIELD_VALUE;
