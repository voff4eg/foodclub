...
class CUser extends CAllUser
{
	...
	function CUser()
	{
		//регистрируем сессионный массив - в нем будем все хранить
//		global $SESS_AUTH;
...
//		$this->SESS_AUTH=&$SESS_AUTH;
	...
	function GetList(&$by, &$order, $arFilter=Array(), $arParams=Array())
	{
			...
			foreach ($arFilter as $key => $val)
			{
				...
				case "CONFIRM_CODE":
					if(strlen($val) > 0)
						...
						$arSqlSearch[] = "U.CONFIRM_CODE IS NULL";
					break;
				case "!CONFIRM_CODE":
					if(strlen($val) > 0)
						$arSqlSearch[] = "U.CONFIRM_CODE <> '".$DB->ForSql($val)."'";
					else
						$arSqlSearch[] = "U.CONFIRM_CODE IS NOT NULL OR U.CONFIRM_CODE = ''";
					break;
				case "COUNTRY_ID":
				...
				case "GROUPS_ID":
					...
					{
						$from1 .= " INNER JOIN b_user_group UG ON (UG.USER_ID=U.ID) ";
						$str = implode(" | ",$val);
						$arSqlSearch[] = GetFilterQuery("UG.GROUP_ID", $str, "N");
						$arSqlSearch[] = "((UG.DATE_ACTIVE_FROM IS NULL) OR (UG.DATE_ACTIVE_FROM <= ".$DB->CurrentTimeFunction()."))";
						$arSqlSearch[] = "((UG.DATE_ACTIVE_TO IS NULL) OR (UG.DATE_ACTIVE_TO >= ".$DB->CurrentTimeFunction()."))";
				...
				case "CHECK_SUBORDINATE":
					$strSubord = "0";
					for($isub=0; $isub<count($val); $isub++)
						$strSubord .= ",".IntVal($val[$isub]);
					$from1 .= " LEFT JOIN b_user_group UGS ON UGS.USER_ID=U.ID AND UGS.GROUP_ID NOT IN ($strSubord) ";
					if($arFilter["CHECK_SUBORDINATE_AND_OWN"] > 0)
						$arSqlSearch[] = "(UGS.GROUP_ID IS NULL OR U.ID=".intval($arFilter["CHECK_SUBORDINATE_AND_OWN"]).")";
					else
						$arSqlSearch[] = "UGS.GROUP_ID IS NULL";
				...
				case "NOT_ADMIN":
					if($val !== true)
						break;
					$from1 .= " LEFT JOIN b_user_group UGNA ON (UGNA.USER_ID=U.ID AND UGNA.GROUP_ID = 1) ";
					$arSqlSearch[] = "UGNA.GROUP_ID IS NULL";
		...
		if($by == "CURRENT_BIRTHDAY")
		{
			...
			$strSqlOrder = " ORDER BY IF (
				STR_TO_DATE(DATE_FORMAT(U.PERSONAL_BIRTHDAY, '".$cur_year."-%m-%d'), '%Y-%m-%d') < CURDATE(),
				STR_TO_DATE(DATE_FORMAT(U.PERSONAL_BIRTHDAY, '".($cur_year + 1)."-%m-%d'), '%Y-%m-%d'),
				STR_TO_DATE(DATE_FORMAT(U.PERSONAL_BIRTHDAY, '".$cur_year."-%m-%d'), '%Y-%m-%d')
			) ";
		}
		...
		$sSelect = ($bGroupBy || $obUserFieldsSql->GetDistinct() ? "DISTINCT" : "")."
			U.*,
		...
		{
			$res_cnt = $DB->Query("SELECT COUNT(".($bGroupBy? "DISTINCT U.ID": "'x'").") as C ".$strSql);
