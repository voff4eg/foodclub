...
$liveid_disabled = !(function_exists('mcrypt_decrypt') && function_exists('mhash')) ? 'Y' : 'N';
...
$arAllOptions = array(
	...
	"update" => Array(
		...
		Array("update_site_proxy_pass", GetMessage("MAIN_UPDATE_SERVER_PR_PS"), "", Array("text", 30)),
...
<?
if(!IsModuleInstalled("controller") && $REQUEST_METHOD=="POST" && strlen($controller_join)>0 && $USER->IsAdmin() && check_bitrix_sessid() && COption::GetOptionString("main", "controller_member", "N")!="Y")
{
...
if(!IsModuleInstalled("controller") && $REQUEST_METHOD=="POST" && strlen($controller_remove)>0 && $USER->IsAdmin() && check_bitrix_sessid() && COption::GetOptionString("main", "controller_member", "N")=="Y")
{
...
function DoNext(name, id, recount)
{
	...
	if (result['stop'] == false)
	{
		if (result['done'] == true)
		{
			...
			if (recount == 'begin')
				str = '&recount=begin';
			document.getElementById('frame_disk_quota').src='/bitrix/admin/quota.php?name=' + id + '&id=' + name + str;
		}
	}
	else
	{
