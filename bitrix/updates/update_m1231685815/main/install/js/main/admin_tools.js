...
function JCAdminMenu(sOpenedSections)
{
	...
	this.ToggleDynSection = function(cell, module_id, div_id, level)
	{
		...
		if(div.innerHTML == '')
		{
			...
			this.request.Send('/bitrix/admin/get_menu.php?lang='+phpVars.LANGUAGE_ID+'&admin_mnu_module_id='+module_id+'&admin_mnu_menu_id='+jsUtils.urlencode(div_id));
		}
...
function TabControl(name, unique_name, aTabs)
{
	...
	this.SaveSettings =  function()
	{
		...
		if(oSelect)
		{
			...
			for(var i=0; i<k; i++)
			{
				...
				if(oFieldsSelect)
				{
					...
					for(var j=0; j<n; j++)
					{
						s += '--,--' + oFieldsSelect[j].value + '--#--' + oFieldsSelect[j].text;
					}
				}
				sTabs += s + '--;--';
			}
		...
		jsUserOptions.DeleteOption('form', this.name+'_disabled', false);
		...
		request.Post('/bitrix/admin/user_options.php?lang=' + phpVars.LANGUAGE_ID, sParam);
	}
	...
	this.DeleteSettings = function(bCommon)
	{
		ShowWaitWindow();
		var url = window.location.href;
		jsUserOptions.DeleteOption('form', this.name, bCommon);
		window.location = url;
	}
	...
	this.DisableSettings = function()
	{
		...
		request.Send('/bitrix/admin/user_options.php?lang=' + phpVars.LANGUAGE_ID + sParam);
	}
...
function JCPanel()
{
	...
	this.ShowOn = function()
	{
		...
		if(bIE)
		{
			...
			{
//				document.body.style.backgroundImage = "url(javascript:false;)";
...
function CWizardWindow()
{
	...
	this.Open = function(wizardName, sessid)
	{
		...
		if (this.iframe.addEventListener)
			this.iframe.addEventListener("load",_this.OnFrameLoad, true);
		else if (this.iframe.attachEvent)
			this.iframe.attachEvent("onload", _this.OnFrameLoad);
	}
...
function JCStartMenu()
{
	...
	this.ShowStartMenu = function(button, back_url)
	{
		...
		if(!menuStart)
		{
			...
			request.Send('/bitrix/admin/get_start_menu.php?lang='+phpVars.LANGUAGE_ID+(back_url? '&back_url_pub='+encodeURIComponent(back_url):''));
		}
		else
		{
	...
	this.OpenDynMenu = function(menu, module_id, items_id, back_url)
	{
		...
		request.Send('/bitrix/admin/get_start_menu.php?mode=dynamic&lang='+phpVars.LANGUAGE_ID+'&admin_mnu_module_id='+jsUtils.urlencode(module_id)+'&admin_mnu_menu_id='+jsUtils.urlencode(items_id)+(back_url? '&back_url_pub='+encodeURIComponent(back_url):''));
	}
	...
	this.OpenURL = function(item, arguments, url, back_url)
	{
		...
		{
			request.Action = function(result){}
			request.Send('/bitrix/admin/get_start_menu.php?mode=save_recent&url='+encodeURIComponent(url)+'&text='+jsUtils.urlencode(itemInfo['TEXT'])+'&title='+jsUtils.urlencode(itemInfo['TITLE'])+'&icon='+itemInfo['ICON']);
		}
		if(back_url)
