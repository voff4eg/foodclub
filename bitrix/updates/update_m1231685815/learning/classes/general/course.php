...
class CAllCourse
{
	...
	function Delete($ID)
	{
		...
		$strSql = "SELECT PREVIEW_PICTURE FROM b_learn_course WHERE ID = ".$ID;
	...
	function GetCourseContent($COURSE_ID, $arAddSelectFileds = Array("DETAIL_TEXT", "DETAIL_TEXT_TYPE", "DETAIL_PICTURE"))
	{
		...
		$strSql =
		"(SELECT ".$strSelect." 'LE' as TYPE".$strAddtional." FROM b_learn_lesson WHERE COURSE_ID = '".$COURSE_ID."' AND ACTIVE = 'Y') ".
		"UNION ALL ".
		"(SELECT ".$strSelect." 'CH' as TYPE".$strAddtional." FROM b_learn_chapter WHERE COURSE_ID = '".$COURSE_ID."' AND ACTIVE = 'Y') ".
