...
foreach ($arResult["DATA"]["HEADER"] as $res):
	if ($res["HREF"] == "Y"):
				?><td class="href"><a href="<?=$res["LINK"]?>"><?=$res["TITLE"]?></a></td><?
	else:
?>
				...
				?>}" id="header_<?=$iObjectID?>_<?=$res["ID"]?>">
					<table cellpadding="0" cellspacing="0" border="0" width="100%" class="tab-header">
						...
						<tr class="middle">
							...
							<?if (!empty($res["LINK"])):?>
							<a href="<?=$res["LINK"]?>" onclick="return false;">
							<?=$res["TITLE"]?></a>
							<?else:?>
							<?=$res["TITLE"]?>
							<?endif;?></div></td>
...
<?
	if (!empty($res["LINK"]) && $res["AJAX_USE"] == "Y"):
		?><div class="photo-body-link"><a href="<?=$res["LINK"]?>" id="text_a_<?=$iObjectID?>_<?=$res["ID"]?>"><?=GetMessage("P_GO_TO_PAGE")?></a></div><?
	endif;
