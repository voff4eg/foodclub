...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true) {        
        ...
        $this->options = array(
            ...
            'upload_dir' => $_SERVER["DOCUMENT_ROOT"].'/upload/tmp/',
            'upload_url' => $this->get_full_url().'/upload/tmp/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    /*if (!empty($_FILES['Filedata']['tmp_name'])) {
                        ...
                        $oUser->Update($iUserID, $aFields);
                    }*/  

                    /*echo $file_path."@";
                    echo "<pre>";print_r(fopen($uploaded_file, 'r'));echo "</pre>";die;*/
                    ...
                    //echo "file_path = ".$file_path;echo " uploaded_file = ".$uploaded_file;die;
                    move_uploaded_file($uploaded_file, $file_path);
                }
            } else {
                // Non-multipart uploads (PUT method support)
               /* echo $file_path."@";
                echo "<pre>";print_r($append_file);echo "</pre>";
                echo "<pre>";print_r(fopen('php://input', 'r'));echo "</pre>";die;*/
            ...
            //if (file_exists($this->options["upload_url"].$file->name)) {
            if (file_exists($file_path)) {
                //echo "@".$this->options["upload_url"].$file->name."@";                
                $oUser = new CUser;
                $rsUser = $oUser->GetByID($oUser->GetID());
                if($arUser = $rsUser->Fetch()){
                    ...
                    "type" =>  $file->type);*/
                    /*$arFile['del'] = "Y";           
                    $arFile['old_file'] = $arUser['UF_BACKGROUND'];
                    $arFile["MODULE_ID"] = "main"; */                   

                    //$arResizedFile = CFile::ResizeImage(&$arFile, array("width" => "960", "height" => "135"), $resizeType = BX_RESIZE_IMAGE_PROPORTIONAL);
                    $checkfile = CFile::CheckFile($arFile,900000,'image/','gif,png,jpeg,jpg');
                    if(strlen($checkfile) > 0) {
                        $strError .= $arFile['name'].': '.$strError.'<br>';die;
                    } else {
                        ...
                        $arResizedFile = CFile::MakeFileArray($_SERVER["DOCUMENT_ROOT"].CFile::GetPath($fileId));
                        $arResizedFile['del'] = "Y";
                        $arResizedFile['old_file'] = $arUser['PERSONAL_PHOTO'];
                        //$arResizedFile["MODULE_ID"] = "main";
                        $fields['PERSONAL_PHOTO'] = $arResizedFile;

                        $aFields = array(
                            'PERSONAL_PHOTO' => $fields['PERSONAL_PHOTO']// <- обязательно array
                        );                        
                        $oUser->Update($arUser["ID"], $aFields); //$iUserID (int) ID of USER
                        $strError = "";
                        $strError .= $oUser->LAST_ERROR;
                        if(strlen($strError)){
                            echo $strError;die;
                        } else{                            
                            $rsUser = $oUser->GetByID($arUser["ID"]);
                            if($arUser = $rsUser->Fetch()){
                                //echo $file_path."@".CFile::GetPath($arUser["UF_BACKGROUND"]);
                                if(intval($arUser["PERSONAL_PHOTO"])){
                                    $file_path = $_SERVER["DOCUMENT_ROOT"].CFile::GetPath($arUser["PERSONAL_PHOTO"]);
                                    $fileArray = CFile::MakeFileArray($file_path);
                                    $rsFile = CFile::GetByID($arUser["PERSONAL_PHOTO"]);
                                    $arFile = $rsFile->Fetch();
                                    //echo "<pre>";print_R($arFile);echo "</pre>";die;
                                    $file->size = $fileArray["size"];
                                    $file->name = $fileArray["name"];
                                    $this->options["upload_url"] = "http://".$_SERVER["SERVER_NAME"]."/upload/".$arFile["SUBDIR"]."/";
                                    //echo print_R($append_file);die;
                                    unlink($arResizedFile["tmp_name"]);
                                }
                            }
                            global $CACHE_MANAGER;
                            $CACHE_MANAGER->ClearByTag("profile#".$arUser["ID"]);
                            if(intval($arUser["UF_RAITING"]) > 160){
                                $CACHE_MANAGER->ClearByTag("users_index");
                                $CACHE_MANAGER->ClearByTag("users_not_index");
                            }
                        }                      
                    }                    
                    //echo "<pre>";print_R($arResizedFile);echo "</pre>";die;                                        
                }
            }else{
                echo "not exist ".$this->options["upload_url"].$file->name;die;
            }
            ...
            if ($file_size === $file->size) {
                ...
                //echo "unlink file_size=".$file_size." file->size=".$file->size;die;
                unlink($file_path);
                /*$oUser = new CUser;
                $rsUser = $oUser->GetByID($oUser->GetID());
                if($arUser = $rsUser->Fetch()){
